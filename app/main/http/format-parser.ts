...
function extractFloat (str: string) {
  const match = /(\d+(\.\d+)?)/.exec(str)
  return match ? parseFloat(match[0]) : 0
}

function extractSizeText (str: string) {
...
export default {
      ...
      if (/yesterday|昨天/.test(dateText)) {
        ...
        for (let i = 0; i < unit.length; i++) {
          ...
          if (dateTextMatches) {
            ...
            const name = unit[i].name as DurationInputArg2
   ...
   * @param sizeText
   */
  extractFileSize (sizeText: string) {
